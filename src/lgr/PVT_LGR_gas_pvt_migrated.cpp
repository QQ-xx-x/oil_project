...
PYBIND11_MODULE(gaspvt, m) {
    py::class_<SimulatorLGR>(m, "SimulatorLGR")
        .def(py::init<>())
        .def("buildGasPVTTable", &SimulatorLGR::buildGasPVTTable)
        .def("preprocess", &SimulatorLGR::preprocess)
        .def("run", &SimulatorLGR::run)
        .def("initState", &SimulatorLGR::initState)
        .def("setupWells", &SimulatorLGR::setupWells)
        .def("exportStaticGeometry", &SimulatorLGR::exportStaticGeometry)
        .def("generateFractures", &SimulatorLGR::generateFractures)
        .def("generateHydraulicFractures", &SimulatorLGR::generateHydraulicFractures);

    m.def("test", []() { return 666; });
}