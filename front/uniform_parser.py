...
def _is_property_keyword_token(token: str) -> bool:
    cleaned = token.strip().strip("'\"")
    if cleaned.endswith('/'):
        cleaned = cleaned[:-1].strip()
    upper = cleaned.upper()
    if upper in PROPERTY_KEYWORDS:
        return True
    return upper.startswith('MATRIX_') or upper.startswith('DFN_')


def _split_property_data_line(line: str) -> Tuple[List[str], bool]:
    """Return numeric/RLE tokens from one property line and whether '/' ended it."""
    line = line.split('--', 1)[0].strip()
    if not line:
        return [], False
    line = re.sub(r'/\s*$', ' /', line)
    tokens = _split_tokens(line)
    if tokens and _is_property_keyword_token(tokens[0]):
        tokens = tokens[1:]
...
def parse_property(path: str, expected_len: Optional[int] = None) -> List[float]:
    ...
    for line in lines:
        ...
        if 'Null Value' in stripped and '=' in stripped:
            continue
        tokens, ended = _split_property_data_line(stripped)
        data_tokens.extend(tokens)
        if ended:
            break

    data_text = ' '.join(data_tokens)
...
def _parse_completion_row(row: Dict[str, str], row_idx: int,
    ...
    if event != 'PERF':
        if _is_blank(row.get('is_fractured')):
            is_fractured = None
        if _is_blank(row.get('connect_matrix')):
            connect_matrix = None
        if _is_blank(row.get('connect_fracture')):
            connect_fracture = None
        if _is_blank(row.get('frac_id')):
            frac_id = None
    ...
    if event == 'PERF':
        ...
        if any(value is None for value in (md_top, md_bottom)) and (
                not _is_blank(row.get('md_top_m')) or not _is_blank(row.get('md_bottom_m'))):
            raise ValueError(f"well_completions row {row_idx}: md_top_m and md_bottom_m must both be provided")
        if md_top is not None and md_bottom is not None and md_top >= md_bottom:
            raise ValueError(f"well_completions row {row_idx}: md_top_m must be smaller than md_bottom_m")
        if rw_m is not None and rw_m <= 0:
            raise ValueError(f"well_completions row {row_idx}: rw_m must be positive")
    elif event == 'CONTROL':
        if bhp_bar is None:
            raise ValueError(f"well_completions row {row_idx}: CONTROL requires bhp_bar")

    well = wells[name]
    if md_top is not None and md_bottom is not None and (
            md_top < well['md_min_m'] - 1e-9 or md_bottom > well['md_max_m'] + 1e-9):
        raise ValueError(
    ...
    segment = None
    if md_top is not None and md_bottom is not None:
        segment = _build_well_segment(well['track'], md_top, md_bottom)
...
def _build_well_event(parsed: Dict[str, Any], event_id: int,
    ...
    referenced = parsed['comp_id'] if parsed['comp_id'] and key in definitions else ''
    return {
        'event_id': event_id,
...
def _inherit_completion_values(parsed: Dict[str, Any],
                               definition: Dict[str, Any]) -> Dict[str, Any]:
    inherited = dict(parsed)
    for key in (
            'well_type', 'is_fractured', 'md_top_m', 'md_bottom_m',
            'depth_type', 'rw_m', 'control_type', 'bhp_bar',
            'connect_matrix', 'connect_fracture', 'connection_target',
            'frac_id', 'well_segment_xyz'):
    ...
    if inherited.get('control_type') in (None, ''):
        inherited['control_type'] = definition.get('control_type', 'BHP')
    return inherited
...
def parse_wells(track_file: str, completion_file: str) -> Dict[str, Any]:
    ...
    for parsed in parsed_rows:
        ...
        if event in {'OPEN', 'SHUT'}:
            ...
            parsed = _inherit_completion_values(parsed, definitions[key])
            if event == 'OPEN':
            ...
            if parsed['comp_id'] and key in definitions:
                parsed = _inherit_completion_values(parsed, definitions[key])
            elif parsed['comp_id']:
                warnings.append(
