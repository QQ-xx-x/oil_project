...
def _apply_case_data_grid_file(sim, params):
    """从 case_dataset 的 arrays.npz 加载网格数组，调用 setCornerPointGrid。"""
    ds_path = params.get('case_dataset_path', '')
    if not ds_path:
    ...
    ds = load_case_dataset(ds_path)
    g = ds.grid
    sim.setCornerPointGrid(g.nx, g.ny, g.nz, g.coord, g.zcorn, g.actnum)


def _apply_case_data_property_files(sim, params):
    """从 case_dataset 加载 MATRIX / DFN / SIGMA 数组，调用 C++ setter。"""
    ds_path = params.get('case_dataset_path', '')
    if not ds_path:
        return
    ...
    # Matrix continuum properties
    if hasattr(sim, 'setMatrixContinuumProperties'):
        sim.setMatrixContinuumProperties(
            list(ds.properties.get('matrix_phi', [])),
            list(ds.properties.get('matrix_kx', [])),
            list(ds.properties.get('matrix_ky', [])),
            list(ds.properties.get('matrix_kz', [])),
        )

    # DFN continuum properties
    if hasattr(sim, 'setDFNContinuumProperties'):
        fphi = ds.properties.get('fracture_phi', None)
        fkx  = ds.properties.get('fracture_kx', None)
        fky  = ds.properties.get('fracture_ky', None)
        fkz  = ds.properties.get('fracture_kz', None)
        if fphi is not None and len(fphi) > 0:
            mask = np.ones(len(fphi), dtype=bool)
            g = ds.grid
            sim.setDFNContinuumProperties(g.nx, g.ny, g.nz, fphi, fkx, fky, fkz, mask)

    # Sigma
    if hasattr(sim, 'setSigmaArray'):
        sigma = ds.properties.get('sigma', None)
        if sigma is not None and len(sigma) > 0:
            sim.setSigmaArray(list(sigma))


def _apply_case_data_fracture_file(sim, params):
    """DFN 几何暂不通过 case_dataset 传入，仍用随机生成 fallback。"""
    # TODO: 适配 dev/w 的 setDFNFractures(ids, offsets, vertices, apertures, perms)
    pass
